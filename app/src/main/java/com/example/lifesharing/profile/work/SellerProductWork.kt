...
/** 대여자 프로필 - 대여 물품 조회 Model */
class SellerProductWork {
    private val retrofitService = RetrofitAPIwithToken.retrofit()
    val TAG: String = "대여자 등록 물품 로그"

    // 대여자가 등록한 물품 조회
    fun getSellerProduct(sellerId : Long, callback: (SellerProductResponse?, String?) -> Unit){
        retrofitService.getSellerProducts(sellerId).enqueue(object : retrofit2.Callback<SellerProductResponse>{
            override fun onResponse(
                ...
                if (response.isSuccessful){
                    ...
                    Log.d(TAG, "대여자 등록제품 조회 성공: $result")
                }
                else{
                    callback(null, "대여자 등록제품 조회 실패: ${response.body()}")
                    Log.e(TAG, "대여자 등록제품 조회 실패: $response")
                }
            }

            override fun onFailure(call: Call<SellerProductResponse>, t: Throwable) {
                callback(null, "대여자 등록제품 조회 실패: ${t.message}")
